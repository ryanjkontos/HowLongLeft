...
class EventUICountdownViewController: NSViewController {
    ...
    @objc func mainRun() {
        ...
        if let currentEvent = self.parentController.event?.getUpdatedInstance(){
            ...
            eventText.stringValue = "\(currentEvent.title) \(currentEvent.countdownTypeString)"
            
            self.timerLabel.stringValue = CountdownStringGenerator.generatePositionalCountdown(event: currentEvent, allowFullUnits: true)
            
            self.timerLabel.textColor = currentEvent.color
            
            progressBar.doubleValue = currentEvent.completionFraction*100
            
            if currentEvent.completionFraction > 0, let percent = PercentageCalculator.optionalCalculatePercentageDone(for: currentEvent) {

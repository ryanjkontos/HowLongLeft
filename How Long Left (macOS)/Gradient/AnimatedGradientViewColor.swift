...
public enum AnimatedGradientViewColor {
    ...
    public var nsColor: NSColor {
        ...
        case .named(let colorName):
            if #available(OSX 10.13, *) {
                if let color = NSColor(named: colorName) {
                    return color
                }
            }
            return type(of: self).colorNameMap[colorName]! // Previously validated
        case .hex(let hexString):
            return type(of: self).parseHex(hexString: hexString)!.nsColor // Previously validated
    ...
    public init?(string: String) {
        ...
        default:
            if #available(OSX 10.13, *) {
                if NSColor(named: string) != nil { // Validate a color is returned

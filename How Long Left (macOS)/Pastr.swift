...
public extension Pastr {
    ...
    static func post(text: String, name: String? = nil, scope: PastrScope = .unlisted, format: String? = nil, expiration: PastrExpiration = .never, completion: @escaping (PastrResult) -> ()) {
    ...
    static func get(paste key: String, isPrivate: Bool = false, completion: @escaping (PastrResult) -> ()) {
        ...
        if isPrivate {
            ...
            URL(string: "https://pastebin.com/raw/\(key)")?.execute { completion($0) }
        }
    }
    ...
    static func delete(paste key: String, completion: @escaping (PastrResult) -> ()) {
    ...
    static func login(username: String, password: String, completion: @escaping (PastrResult) -> ()) {
    ...
    static func getUserPastes(limit: Int? = nil, completion: @escaping (PastrResult) -> ()) {
    ...
    /// - parameter completion: A closure passing a `PastrResult` enum containing either the list of pastes in raw xml or an error
    /// TODO: This will just return unparsed XML in a string for now
    static func getTrendingPastes(completion: @escaping (PastrResult) -> ()) {
    ...
    /// - note: Usage of this API requires a user key
    /// TODO: This will just return unparsed XML in a string for now
    static func getUserInfo(completion: @escaping (PastrResult) -> ()) {
...
private extension URL {
    ...
    init(_ endpoint: PastebinEndpoint) {
        self.init(string: "https://pastebin.com/api/\(endpoint.rawValue).php")!
    }

...
class HLLTimelineGenerator {
    ...
    func generateHLLTimeline(fast: Bool = false, percentages: Bool = false, forState: TimelineState = .normal) -> HLLTimeline {
            ...
            var events = Array(HLLEventSource.shared.events.filter({HLLStoredEventManager.shared.isHidden(event: $0) == false})).filter({ $0.completionStatus != .done })
    ...
    func getNextEventToStartOrEnd(at date: Date, from events: [HLLEvent]) -> HLLEvent? {
        ...
        if doSelected {
            if let selected = HLLEventSource.shared.getPinnedEventsFromevents().first {
